...
2. Review [the worksheet](LAB6_ERROR_REVIEW.md). Record your chosen `category`, `reviewer_note`, and `human_confirmed: true` in `artifacts/lab6/human_error_review.json`. Only explicitly confirmed entries count; no automatic taxonomy tags are treated as human work.
